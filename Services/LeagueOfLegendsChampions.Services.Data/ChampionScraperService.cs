namespace LeagueOfLegendsChampions.Services.Data
{
    ...
    public class ChampionScraperService : IChampionScraperService
    {
        ...
        public async Task ImportChampionsNamesAndIconsAsync()
        {
            ...
            foreach (var currentChampion in docForChampions.DocumentNode.SelectNodes("//div[@class='ChampGrid__ChampGridCell-cz5mrj-1 jXHsjY']"))
            {
                string url = currentChampion.InnerHtml.Trim().Replace(" ", string.Empty);
                int indexOfUrlFirst = url.IndexOf("src=");
                int indexOfUrlSecond = url.IndexOf(".png");
                string championIconImageUrl = url.Substring(indexOfUrlFirst + 4, indexOfUrlSecond - indexOfUrlFirst).Replace("('", string.Empty).Replace("\"", string.Empty);
                int indexOfUrlFirstName = url.IndexOf("alt=");
                int indexOfUrlSecondName = url.IndexOf("\"></a");
                string championNameFull = url.Substring(indexOfUrlFirstName + 4, indexOfUrlSecondName - indexOfUrlFirstName).Replace("('", string.Empty).Replace("\"", string.Empty).ToString().Replace("></", string.Empty).Replace("&#x27;", string.Empty).Replace("&amp;", " & ");
